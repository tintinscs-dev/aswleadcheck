...
const ACTION_LABEL = {
  ...
  adjustment_proposed: 'De xuat dieu chinh phi — cho Manager duyet',
  adjustment_approved: 'Manager da duyet dieu chinh phi',
  adjustment_rejected: 'Manager tu choi dieu chinh phi',
};
...
const ACTION_COLOR = {
  ...
  approved:            '#1D7A4F',
  rejected:             '#C0392B',
  adjustment_proposed: '#C98A1F',
  adjustment_approved: '#1D7A4F',
  adjustment_rejected: '#C0392B',
};
...
async function getRecipients(quote, action) {
  ...
  if (action === 'approved') {
    // Manager đã duyệt → báo cho Kế toán + Ops để chuẩn bị chứng từ / thực hiện lô hàng
    const opsAcc = await prisma.user.findMany({
      where: { role: { in: ['accounting', 'operation'] }, notifyEmail: { not: null } },
      select: { notifyEmail: true },
    });
    opsAcc.forEach(u => u.notifyEmail && emails.push(u.notifyEmail));
  }

  if (action === 'adjustment_proposed') {
    // Manager + Admin cần duyệt đề xuất điều chỉnh
    const users = await prisma.user.findMany({
      where: { role: { in: ['manager', 'admin'] }, notifyEmail: { not: null } },
      select: { notifyEmail: true },
    });
  ...
  if (['adjustment_approved', 'adjustment_rejected'].includes(action)) {
    // Người đã đề xuất điều chỉnh + Admin
    if (quote.pendingAdjustment?.proposedById) {
      const proposer = await prisma.user.findUnique({
        where: { id: quote.pendingAdjustment.proposedById },
        select: { notifyEmail: true },
      });
      if (proposer?.notifyEmail) emails.push(proposer.notifyEmail);
    }
    const admins = await prisma.user.findMany({
      where: { role: 'admin', notifyEmail: { not: null } },
      select: { notifyEmail: true },
