...
import {
  MODE_LABELS, itemDefsForMode,
  qtyForMode, lineTotal, quoteModes, calcQuote, fmt, statusLabel, itemCurrency,
} from './calc';
...
// Column layout for the cost/sell tables. VAT% is intentionally not shown as its
// own column (it's still applied inside "Thành tiền") — dropping it frees up room
// so the remaining columns aren't cramped/overlapping.
const COST_TABLE_COLS = [
  { label: 'Hạng mục',    frac: 0.30, align: 'left' },
  { label: 'Đơn giá',     frac: 0.15, align: 'right' },
  { label: 'Đơn vị tính', frac: 0.14, align: 'right' },
  { label: 'Tiền',        frac: 0.09, align: 'right' },
  { label: 'Thành tiền',  frac: 0.16, align: 'right' },
  { label: 'Ghi chú',     frac: 0.16, align: 'left' },
];

function drawCostTable(doc, x, y, title, side, mode, q, maxWidth) {
  const colWidths = COST_TABLE_COLS.map(c => maxWidth * c.frac);

  doc.font('RobotoBold').fontSize(10).fillColor('#0b2545').text(title, x, y);
  y += 16;

  y = drawTableRow(doc, x, y, COST_TABLE_COLS.map((c, i) => ({ text: c.label, width: colWidths[i], align: c.align })), { bold: true, fill: '#eef2f8' });

  const qty = qtyForMode(mode, q);
  const data = q[side][mode] || {};
  const rows = [];
  itemDefsForMode(mode).forEach(d => rows.push([d.label, data[d.key] || {}]));
  (data.customItems || []).forEach(ci => rows.push([ci.label || '(Hạng mục tự thêm)', ci]));
  const visibleRows = rows.filter(([, item]) => Number(item?.price || 0));

  let zebra = false;
  if (!visibleRows.length) {
    y = drawTableRow(doc, x, y, [{ text: 'Không có chi phí nào được nhập.', width: maxWidth }], { fontSize: 8, color: '#8a8f98' });
    return y + 6;
  }
  visibleRows.forEach(([label, item]) => {
    const total = lineTotal(item, qty, +1);
    y = drawTableRow(doc, x, y, [
      { text: label, width: colWidths[0], align: 'left' },
      { text: money(item.price || 0), width: colWidths[1], align: 'right' },
      { text: item.unit || '', width: colWidths[2], align: 'right' },
      { text: itemCurrency(item), width: colWidths[3], align: 'right' },
      { text: money(total), width: colWidths[4], align: 'right' },
      { text: item.note || '', width: colWidths[5], align: 'left' },
...
// "Overseas Charges" — free-form charges at the foreign end for export/import shipments
// (Destination/Origin THC, overseas customs, overseas trucking...). Only drawn when the
// quote actually has at least one such line, so most shipments never show an empty table.
function drawOverseasTable(doc, x, y, title, side, mode, q, maxWidth) {
  const data = q[side]?.[mode] || {};
  const items = (data.overseasItems || []).filter(oi => Number(oi?.price || 0));
  ...
  items.forEach(item => {
    const total = lineTotal(item, qty, +1);
    y = drawTableRow(doc, x, y, [
      { text: item.label || '(Chưa đặt tên)', width: colWidths[0], align: 'left' },
      { text: money(item.price || 0), width: colWidths[1], align: 'right' },
      { text: item.unit || '', width: colWidths[2], align: 'right' },
      { text: itemCurrency(item), width: colWidths[3], align: 'right' },
      { text: money(total), width: colWidths[4], align: 'right' },
...
export async function buildQuotePdf(quote, exportedByName) {
  ...
  const infoLines = [
    ...
    [`20': ${q.qty20 || 0}`, `40': ${q.qty40 || 0}`, `CBM: ${q.lcl || 0}`, `G.W(kg): ${q.weight || 0}`, `C.W(kg): ${q.cw || 0}`, `PKGS: ${q.pieces || 0}`],
  ];
  ...
  for (const mode of modes) {
    ...
    const hasOverseas = (q.buying?.[mode]?.overseasItems || []).some(oi => Number(oi?.price || 0))
      || (q.selling?.[mode]?.overseasItems || []).some(oi => Number(oi?.price || 0));
    if (hasOverseas) {
      y = ensureSpace(doc, y, 60, PAGE_MARGIN);
      y = drawOverseasTable(doc, PAGE_MARGIN, y, 'I. Overseas Charges — Giá mua (Cost)', 'buying', mode, q, pageWidth);
      y = ensureSpace(doc, y, 60, PAGE_MARGIN);
      y = drawOverseasTable(doc, PAGE_MARGIN, y, 'I. Overseas Charges — Giá bán (Sell)', 'selling', mode, q, pageWidth);
      y += 6;
    }
  ...
  y = ensureSpace(doc, y, 135, PAGE_MARGIN);
  ...
  y += 2;
  doc.font('Roboto').fontSize(7.5).fillColor('#8a8f98')
    .text('CPCN = doanh thu liên quan × lãi suất NH × số ngày nợ / 365 — chi phí cơ hội khi cho khách trả chậm so với gửi ngân hàng. Cược cont & Chi hộ khác chỉ tính phần lãi, không trừ gốc (đã thu lại từ khách).', PAGE_MARGIN, y, { width: pageWidth });
  y = doc.y;
  y += 10;
  ...
  const summaryRows = [
    ...
    ['CKTM (Giá mua)', r.CKTM_LINE], ['CKTM (Giá bán)', r.CKTM_CLIENT], ['Tổng CKTM (đã gồm trong GVDV/DTLH)', r.CKTM_total],
    ['CPCN 0% (OVS/OF)', r.CPCN0], ['CPCN LCC 8%', r.CPCNLCC], ['CPCN CUS+TRUCKING', r.CPCNCusTruck], ['Tổng CPCN', r.CPCN_total],
  ...
  summaryRows.forEach(([label, val], i) => {
    ...
    const yy = y + row * 14;
    doc.font('Roboto').fontSize(8.5).fillColor('#1c2733').text(label, xx, yy, { width: colW * 0.58, lineBreak: false });
    doc.text(`${money(val)} USD`, xx + colW * 0.58, yy, { width: colW * 0.42, align: 'right', lineBreak: false });
  });
  ...
  doc.text(`${money(r.CPLH)} USD`, PAGE_MARGIN, y + 4, { width: pageWidth - 8, align: 'right' });
  const kqkdColor = r.KQKD >= 0 ? '#1d7a4f' : '#c0392b';
  doc.fontSize(12).fillColor(kqkdColor).text('KQKD', PAGE_MARGIN + 8, y + 22);
  doc.text(`${money(r.KQKD)} USD`, PAGE_MARGIN, y + 22, { width: pageWidth - 8, align: 'right' });
