...
function drawCostTable(doc, x, y, title, side, mode, q, maxWidth) {
  const colWidths = [maxWidth * 0.28, maxWidth * 0.15, maxWidth * 0.18, maxWidth * 0.11, maxWidth * 0.1, maxWidth * 0.18];
  const headers = ['Hạng mục', 'Flat', `Đơn giá ${MODE_UNIT[mode]}`, 'VAT/CK%', 'Tiền', 'Thành tiền'];
  ...
  if (side === 'buying') {
    rows.push([COMLINE_DEF.label, { flat: 0, perUnit: data.comline.perUnit, tax: data.comline.tax, currency: data.comline.currency }, -1]);
  } else {
  ...
  const visibleRows = rows.filter(([, item]) => Number(item?.flat || 0) || Number(item?.perUnit || 0));
  ...
  visibleRows.forEach(([label, item, sign]) => {
