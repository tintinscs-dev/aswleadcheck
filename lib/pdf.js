...
function drawCostTable(doc, x, y, title, side, mode, q, maxWidth) {
  const colWidths = [maxWidth * 0.26, maxWidth * 0.14, maxWidth * 0.12, maxWidth * 0.08, maxWidth * 0.08, maxWidth * 0.16, maxWidth * 0.16];
  const headers = ['Hạng mục', 'Đơn giá', 'Đơn vị tính', 'VAT%', 'Tiền', 'Thành tiền', 'Ghi chú'];

  doc.font('RobotoBold').fontSize(10).fillColor('#0b2545').text(title, x, y);
  y += 16;

  y = drawTableRow(doc, x, y, headers.map((h, i) => ({ text: h, width: colWidths[i], align: (i >= 3 && i <= 5) ? 'right' : 'left' })), { bold: true, fill: '#eef2f8' });
  ...
  visibleRows.forEach(([label, item]) => {
    ...
    y = drawTableRow(doc, x, y, [
      ...
      { text: itemCurrency(item), width: colWidths[4], align: 'right' },
      { text: money(total), width: colWidths[5], align: 'right' },
      { text: item.note || '', width: colWidths[6] },
