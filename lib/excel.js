...
function otherRateForMode(side, mode, q) {
  ...
  // OVERSEAS CHARGES lines fold into the same "OTHER" slot as custom items — this legacy
  // sheet has no room for a dedicated section (see file header comment).
  (m.overseasItems || []).forEach(oi => { total += lineTotal(oi, qty, +1); });
  return total / (qty || 1);
}
