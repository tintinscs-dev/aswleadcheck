...
const SECTION_BY_KEY = {
  ...
  // Customs/Trucking no longer has its own printed section — folded into Local Charges.
  truck: 'local', cus: 'local',
};
...
export function quoteToFormalTemplate(quote, opts = {}) {
  ...
  t.extraTerms = Array.isArray(q.extraTerms) ? q.extraTerms.filter(Boolean) : [];
  ...
  modes.forEach(mode => {
    ...
    itemDefsForMode(mode).forEach(d => {
      ...
      t[sectionKey].push({
        ...
        unit: item.unit || '',
        notes: [item.note, item.tax ? `VAT ${item.tax}%` : ''].filter(Boolean).join(' · '),
        // Line total (unit price × quantity, tax included) — see rowAmount() in
        // templateQuote.js. Without this, per-container/per-CBM/per-CW rows summed
        // just the unit price instead of the real total (e.g. OF x 8 containers).
    ...
    (s.customItems || []).forEach(ci => {
      ...
      t.local.push({
        ...
        unit: ci.unit || '',
        notes: [ci.note, ci.tax ? `VAT ${ci.tax}%` : ''].filter(Boolean).join(' · '),
        amount: String(Number(amount.toFixed(2))),
      });
    });

    (s.overseasItems || []).forEach(oi => {
      const amount = lineTotal(oi, qty, +1);
      if (!amount) return;
      t.overseas.push({
        desc: `${oi.label || '(Hạng mục)'}${modeTag}`,
        rate: oi.price ? String(oi.price) : '',
        currency: itemCurrency(oi),
        unit: oi.unit || '',
        notes: [oi.note, oi.tax ? `VAT ${oi.tax}%` : ''].filter(Boolean).join(' · '),
        amount: String(Number(amount.toFixed(2))),
      });
    });
  });

  // 'overseas' is left empty (not force-filled with a blank row) when the shipment has
  // no overseas charges, so the formal PDF/Excel skips that section entirely instead of
  // printing an empty "OVERSEAS CHARGES" table on every quote.
  ['freight', 'local'].forEach(k => { if (!t[k].length) t[k] = [blankChargeRow()]; });
