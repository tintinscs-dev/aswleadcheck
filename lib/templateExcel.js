...
import fs from 'fs';
import { TEMPLATE_SECTIONS, blankTemplate, blankChargeRow, TERMS_GENERAL, TERMS_ADDITIONAL_COSTS, formatRateDisplay } from './templateQuote';
...
export async function buildTemplateWorkbook(template, opts = {}) {
  ...
  TEMPLATE_SECTIONS.forEach(sec => {
    ...
    const totals = {};
    rows.forEach((row, i) => {
      aoa.push([i + 1, row.desc || '', formatRateDisplay(row.rate), row.currency || '', row.unit || '', row.notes || '']);
