...
export async function buildTemplateWorkbook(template, opts = {}) {
  ...
  TEMPLATE_SECTIONS.forEach(sec => {
    // Skip sections with no rows at all (e.g. "OVERSEAS CHARGES" on a shipment with no
    // overseas charges) instead of printing an empty table on the customer-facing export.
    if (!t[sec.key] || !t[sec.key].length) return;
    aoa.push([sec.title]);
    aoa.push(CHARGE_HEADER);
    const rows = t[sec.key];
    const totals = {};
    rows.forEach((row, i) => {
  ...
  ws['!cols'] = [{ wch: 6 }, { wch: 48 }, { wch: 14 }, { wch: 10 }, { wch: 14 }, { wch: 14 }, { wch: 22 }];
  ws['!rows'] = [{ hpt: 40 }];
