...
export function diffQuoteCosts(oldQ, newQ) {
  ...
  MODES.forEach(mode => {
    ['buying', 'selling'].forEach(side => {
      ...
      const oo = o.overseasItems || [], no = n.overseasItems || [];
      const maxLenO = Math.max(oo.length, no.length);
      for (let i = 0; i < maxLenO; i++) {
        const ol = oo[i], nl = no[i];
        if (!ol && nl) out.push(`${MODE_LABELS[mode]} / ${side === 'buying' ? 'Giá mua' : 'Giá bán'} / Overseas Charges / thêm khoản phí mới "${nl.label || '(không tên)'}"`);
        else if (ol && !nl) out.push(`${MODE_LABELS[mode]} / ${side === 'buying' ? 'Giá mua' : 'Giá bán'} / Overseas Charges / xoá khoản phí "${ol.label || '(không tên)'}"`);
        else if (ol && nl) diffItem(side, mode, `Overseas: ${ol.label || `overseas${i}`}`, ol, nl, out);
      }
    });
  });
