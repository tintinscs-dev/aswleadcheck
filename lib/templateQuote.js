...
//   FREIGHT & CHARGES / LOCAL CHARGES Vietnam side / Customs and Trucking in Vietnam side
...
export const TEMPLATE_SECTIONS = [
  ...
  { key: 'local', title: 'LOCAL CHARGES Vietnam side' },
  { key: 'customs', title: 'Customs and Trucking in Vietnam side' },
];
...
function chargeRowToCustomItem(row, exchangeRate) {
  ...
  const item = { label, flat: 0, perUnit: 0, tax: 0 };
  if (amountUsd === null) {
...
export function mapTemplateToQuoteData(template, exchangeRate) {
  ...
  TEMPLATE_SECTIONS.forEach(sec => {
    (template[sec.key] || []).forEach(row => {
      ...
      const item = chargeRowToCustomItem(row, q.exchangeRate);
