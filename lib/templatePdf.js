...
export async function buildTemplatePdf(template, opts = {}) {
  ...
  try {
    if (fs.existsSync(LOGO_PATH)) {
      doc.image(LOGO_PATH, PAGE_MARGIN, y, { height: 30 });
      logoH = 30;
    }
  } catch (e) { /* logo optional */ }
  doc.font('RobotoBold').fontSize(12).fillColor('#0b2545').text(COMPANY.name, PAGE_MARGIN + (logoH ? 120 : 0), y + 2, { width: pageWidth - (logoH ? 120 : 0) });
  doc.font('Roboto').fontSize(7.5).fillColor('#5a6675').text(COMPANY.address, PAGE_MARGIN + (logoH ? 120 : 0), doc.y + 1, { width: pageWidth - (logoH ? 120 : 0) });
  doc.text(`Email: ${COMPANY.email}   •   Tel: ${COMPANY.phone}`, PAGE_MARGIN + (logoH ? 120 : 0), doc.y + 1, { width: pageWidth - (logoH ? 120 : 0) });
  ...
  if (formal) {
    ...
    y = doc.y + 1;
    doc.text(`Email: ${COMPANY.email}   •   Tel: ${COMPANY.phone}`, PAGE_MARGIN, y, { width: pageWidth });
    y = doc.y + 2;
    if (salesName) {
      doc.font('Roboto').fontSize(9).fillColor('#1c2733').text(`Sales: ${salesName}`, PAGE_MARGIN, y);
      y = doc.y + 2;
    }
  }
