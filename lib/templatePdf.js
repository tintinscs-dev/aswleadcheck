...
export async function buildTemplatePdf(template, opts = {}) {
  ...
  doc.font('RobotoBold').fontSize(18).fillColor('#0b2545').text('ASW LOGISTICS', PAGE_MARGIN, y, { width: pageWidth, align: 'center' });
  y = doc.y + 2;
  ...
  if (formal) {
    ...
    y = doc.y + 14;
    doc.font('RobotoBold').fontSize(10).fillColor('#0b2545').text('ASW LOGISTICS', PAGE_MARGIN, y);
    y = doc.y + 2;
    if (salesName) {
