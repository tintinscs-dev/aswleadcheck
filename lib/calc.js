 ...
 * Strictly copies lclair → lcl only; does NOT create blank placeholder objects
 * (the form and print code handle missing keys with || {} fallbacks).
 * Safe to call multiple times (idempotent).
 */
export function migrateQuote(q) {
  if (!q) return q;
  const buying  = q.buying  || {};
  const selling = q.selling || {};
  // Only migrate if old lclair key exists and new lcl key doesn't
...
function modeHasData(q, mode) {
  const hasNonZero = (data) => {
    ...
    return Object.entries(data).some(([k, v]) => {
      if (k === 'customItems') return Array.isArray(v) && v.some(ci => Number(ci?.flat) || Number(ci?.perUnit));
      if (typeof v === 'object' && v !== null && !Array.isArray(v)) {
        return Number(v.flat) !== 0 || Number(v.perUnit) !== 0;
      }
      return false;
    });
  };
...
export function quoteModes(q) {
  ...
  // AIR: weight-only shipment OR charges were entered in air tab (even alongside LCL)
  if ((Number(q.weight) > 0 && !Number(q.qty20) && !Number(q.qty40) && !Number(q.lcl)) || modeHasData(q, 'air')) modes.push('air');
