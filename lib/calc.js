...
export function calcQuote(q) {
  ...
  MODES.forEach(mode => {
    ...
    ITEM_DEFS.forEach(d => { costSub += lineTotal(b[d.key] || zeroItem, qty, +1); });
    ITEM_DEFS.forEach(d => { revSub += lineTotal(s[d.key] || zeroItem, qty, +1); });
    ...
    const comlineVal = (Number(comline.perUnit || 0) * qty) * (1 - (Number(comline.tax || 0) / 100));
    const comVal = lineTotal(com, qty, -1);
    const com10Val = lineTotal(com10, qty, -1);
    GVDV += costSub; DTLH += revSub;
    CKTM_LINE += comlineVal; CKTM_CLIENT += comVal + com10Val;
    baseCPCN0 += (Number(overseas.flat || 0) + Number(overseas.perUnit || 0) * qty) + (Number(ofreight.perUnit || 0) * qty);
    baseLCC += ((Number(thc.perUnit || 0) + Number(cfs.perUnit || 0) + Number(cic.perUnit || 0) + Number(clean.perUnit || 0) + Number(other.perUnit || 0)) * qty + Number(dobill.flat || 0) + Number(hdl.flat || 0) + sCustomSub) * 1.08;
    baseCusTruck += ((Number(truck.perUnit || 0) + Number(cus.perUnit || 0)) * qty) * 1.1;
