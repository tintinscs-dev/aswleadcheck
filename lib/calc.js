...
export function blankItem() { return { flat: 0, perUnit: 0, tax: 0 }; }
export function blankModeBuying() {
  const m = {};
  ITEM_DEFS.forEach(d => (m[d.key] = blankItem()));
  m.comline = { perUnit: 0, tax: 0 };
  m.customItems = [];
  return m;
}
...
export function blankCustomItem() { return { label: '', flat: 0, perUnit: 0, tax: 0 }; }

export function newQuoteData() {
  return {
    no: '', dep: 'FES', keys: '', shpr: '', cnee: '', agent: '',
    pol: '', pod: '', pickup: '', delivery: '', term: '',
    etd: '', eta: '', lineCoLoader: '',
