...
export function blankItem(unit) { return { price: 0, unit: unit || 'Shipment', tax: 0, currency: DEFAULT_CURRENCY, note: '' }; }
...
export function blankCustomItem() { return { label: '', price: 0, unit: 'Shipment', tax: 0, currency: DEFAULT_CURRENCY, note: '' }; }
...
function migrateModeData(mode, data, qty) {
  ...
  const out = {
    customItems: Array.isArray(data.customItems) ? data.customItems.slice() : [],
    overseasItems: Array.isArray(data.overseasItems) ? data.overseasItems.slice() : [],
  };
...
function modeHasData(q, mode) {
  const hasNonZero = (data) => {
    ...
    return Object.entries(data).some(([k, v]) => {
      if (k === 'customItems' || k === 'overseasItems') return Array.isArray(v) && v.some(ci => Number(ci?.price ?? ci?.flat ?? ci?.perUnit));
...
export function calcQuote(q) {
  ...
  allModes.forEach(mode => {
    ...
    (b.overseasItems || []).forEach(oi => { costSub += lineTotal(oi, qty, +1) * rateOf(oi); });
    (s.overseasItems || []).forEach(oi => { revSub  += lineTotal(oi, qty, +1) * rateOf(oi); });
    const sCustomSub = (s.customItems || []).reduce((sum, ci) => sum + lineTotal(ci, qty, +1) * rateOf(ci), 0);
    // Overseas charges are billed to the customer alongside local charges, so they share
    // the same credit-cost bucket/assumption (baseLCC) rather than needing their own
    // credit-days field.
    const sOverseasSub = (s.overseasItems || []).reduce((sum, oi) => sum + lineTotal(oi, qty, +1) * rateOf(oi), 0);
    ...
    if (mode === 'air') {
      ...
      baseLCC += (localSum + sCustomSub + sOverseasSub) * 1.08;
